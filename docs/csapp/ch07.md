...
- **静态库**：通常以.a后缀表示。链接时将目标文件代码复制到每个运行进程中。**缺点：运行时对内存产生极大浪费；更新库需要重新编译发布**
- **动态库**：通常以.so后缀表示。动态库运行时，可以被加载到任意的内存地址，能和内存中的程序链接起来
...
【例】还可以使用`dlopen`函数，运行时动态链接共享库libvector.so
