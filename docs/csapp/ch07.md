...
【链接】可以看到发生了重定位，main.o和sum.o的`.text`段、`.data`段分别合并，且符号指向了运行时地址

```bash
# gcc -Og -o prog main.c sum.c
# objdump -s -d prog
Contents of section .data:
 601028 00000000 00000000 00000000 00000000  ................    #链接器修改array地址，指向正确位置
...
【说明】全局变量`array`和函数`sum`地址需要重定位，这涉及到重定位条目。接下来我们看一下具体的过程
...
- 结构体定义：由下面4个字段组成。type有32种类型，主要关心以下2种：
  - `R_X86_64_PC32`：重定位到**PC相对地址**
  - `R_X86_64_32`：重定位到**PC绝对地址**
...
ref_addr = ADDR(main) + r.offset = 0x4004d0 + 0xf = 0x4004df

*ref_ptr = ADDR(sum) - ref_addr + r.addend = 0x4004e8 - 0x4004df + (-4) = 0x5

PC压入栈中：call下一条指令的地址要+4，所以PC的值是0x4004e3压入栈

跳转到PC的值加上偏移量

sum函数执行完成后，执行ret，需要add $4, %(esp)，
...
【例2】array的引用有**绝对地址重定位**

**sum函数的array实参**赋值如下。这里array的实参地址被汇编器填充为0
...
*ref_ptr = ADDR(array)  + r.addend = 0x601018 + 0 = 0x601018

将array绝对地址赋值到指令，注意使用小端填充
